...
with open("../model_keys/fractionWordProblems.data", 'rb') as f:
    rank_keys = pickle.load(f)

for file_name in os.listdir("../gensim_models2/fractionWordProblems"):
    similarity_data = []
    if not file_name.endswith(".gsm"):
        continue

    full_path = "../gensim_models2/fractionWordProblems/" + file_name
...
for file_name in os.listdir("../gensim_models2/understandingMultiplying"):
    ...
    full_path = "../gensim_models/understandingMultiplying/" + file_name
    problem_type = file_name[:-len(".gsm")].replace('_', ' ')
    model = Word2Vec.load(full_path)
